...
(function () {
  ...
  // Whether rearrange / organise mode is currently active (default: locked/normal)
  let isRearrangeMode = false;

  // Which folder is currently selected in the sidebar (null = show all)
  let activeFilterDayId = null;

  // The exercise ID being dragged (set on dragstart, cleared on dragend/drop)
  ...
  function addWorkoutDay(name) {
    ...
    renderFolderSidebar();
    renderExercises();
    showToast(`Added workout split: ${trimmed}`, 'success');
  }
  ...
  function deleteWorkoutDay(dayId) {
    ...
    if (activeFilterDayId === dayId) activeFilterDayId = null;
    saveState();
    renderWorkoutDaysList();
    renderFolderSidebar();
    renderExercises();
  ...
  /* ==========================================================================
     FOLDER SIDEBAR — render & helper
     ========================================================================== */

  /** Convert a day name into 2–3 character initials for the sidebar icon. */
  function getFolderInitials(name) {
    if (!name) return '?';
  ...
  function renderFolderSidebar() {
    ...
    // Count exercises assigned to each day
    const countMap = {};
    state.workoutDays.forEach(d => { countMap[d.id] = 0; });
    state.exercises.forEach(ex => {
      if (ex.workoutDayId && countMap[ex.workoutDayId] !== undefined) {
        countMap[ex.workoutDayId]++;
      }
    });
    ...
    allBtn.type = 'button';
    allBtn.className = `folder-btn${activeFilterDayId === null ? ' is-active' : ''}`;
    allBtn.title = 'All exercises';
    allBtn.setAttribute('aria-label', 'Show all exercises');
    allBtn.innerHTML = `
      <span class="folder-btn-svg">
        <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
          <rect x="3" y="3" width="7" height="7" rx="1"/><rect x="14" y="3" width="7" height="7" rx="1"/>
          <rect x="3" y="14" width="7" height="7" rx="1"/><rect x="14" y="14" width="7" height="7" rx="1"/>
        </svg>
      </span>
      <span class="folder-btn-label">All</span>
    `;
    // Drag onto "All" to ungrouped
    allBtn.addEventListener('dragover', e => { if (isRearrangeMode) { e.preventDefault(); allBtn.classList.add('drag-over'); } });
    ...
    allBtn.addEventListener('drop', e => {
      ...
      if (!isRearrangeMode || !draggedExerciseId) return;
      const ex = state.exercises.find(x => x.id === draggedExerciseId);
      if (ex) { ex.workoutDayId = null; saveState(); showToast(`Moved "${ex.name}" → Ungrouped`, 'success'); renderFolderSidebar(); renderExercises(); }
      draggedExerciseId = null;
    });
    allBtn.addEventListener('click', () => { activeFilterDayId = null; renderFolderSidebar(); renderExercises(); });
    ...
    // ── One button per workout day ──
    state.workoutDays.forEach(day => {
      const btn = document.createElement('button');
      btn.type = 'button';
      btn.className = `folder-btn${activeFilterDayId === day.id ? ' is-active' : ''}`;
      btn.title = day.name;
      btn.setAttribute('aria-label', `${day.name} folder`);
      btn.dataset.dayId = day.id;

      const initials = getFolderInitials(day.name);
      const count = countMap[day.id] || 0;
      ...
      btn.innerHTML = `
        ...
        <span class="folder-btn-initials">${escapeHtml(initials)}</span>
        <span class="folder-btn-count">${count}</span>
      `;

      // Click to filter
      btn.addEventListener('click', () => { activeFilterDayId = day.id; renderFolderSidebar(); renderExercises(); });

      // Drag-and-drop drop zone
      btn.addEventListener('dragover', e => { if (isRearrangeMode) { e.preventDefault(); btn.classList.add('drag-over'); } });
      ...
      btn.addEventListener('drop', e => {
        ...
        if (!isRearrangeMode || !draggedExerciseId) return;
        const ex = state.exercises.find(x => x.id === draggedExerciseId);
        if (ex) { ex.workoutDayId = day.id; saveState(); showToast(`Moved "${ex.name}" → ${day.name}`, 'success'); renderFolderSidebar(); renderExercises(); }
        draggedExerciseId = null;
      });
    ...
    div2.className = 'folder-sidebar-divider';
    sidebar.appendChild(div2);

    const addBtn = document.createElement('button');
    addBtn.type = 'button';
    addBtn.className = 'folder-btn folder-btn-add';
    addBtn.title = 'Add workout split folder';
    addBtn.setAttribute('aria-label', 'Add new workout split folder');
    addBtn.innerHTML = `
      <span class="folder-btn-svg">
      ...
      <span class="folder-btn-label">Add</span>
    `;
    addBtn.addEventListener('click', () => {
      const name = window.prompt('Name your workout split folder:\n(e.g. Upper, Lower, Push, Pull, Legs, Full Body…)');
      if (name && name.trim()) addWorkoutDay(name.trim());
    });
    sidebar.appendChild(addBtn);
  }

  /* ==========================================================================
     REARRANGE MODE — enter / exit
     ========================================================================== */
  function enterRearrangeMode() {
    isRearrangeMode = true;

    const rearrangeBtn   = document.getElementById('rearrangeBtn');
    const finishBtn      = document.getElementById('finishRearrangeBtn');
    const banner         = document.getElementById('rearrangeBanner');
    const grid           = document.getElementById('exercisesGrid');

    if (rearrangeBtn)  rearrangeBtn.style.display  = 'none';
    if (finishBtn)     finishBtn.style.display      = 'inline-flex';
    if (banner)        banner.style.display         = 'flex';
    if (grid)          grid.classList.add('is-rearranging');

    // Collapse any open card so the header shows the day select cleanly
    if (activeCardId) {
  ...
  function exitRearrangeMode() {
    ...
    if (rearrangeBtn)  rearrangeBtn.style.display  = 'inline-flex';
    if (finishBtn)     finishBtn.style.display      = 'none';
    if (banner)        banner.style.display         = 'none';
    if (grid)          grid.classList.remove('is-rearranging');

    renderFolderSidebar();
    renderExercises();
    showToast('Organisation saved — normal mode restored', 'success');
  }
  ...
  function addExercise(name) {
    ...
    const newExercise = {
      ...
      workoutDayId: activeFilterDayId || null,
    ...
    renderExercises();
    updateSummaryBar();
    const folderLabel = activeFilterDayId
      ? (state.workoutDays.find(d => d.id === activeFilterDayId)?.name || 'folder')
      : null;
    showToast(folderLabel ? `Added "${trimmed}" to ${folderLabel}` : `Added "${trimmed}" to your exercises`, 'success');
  }
  ...
  function renderExercises() {
    ...
    const organizeToolbar = document.getElementById('organizeToolbar');
    ...
    if (state.exercises.length === 0) {
      ...
      if (organizeToolbar) organizeToolbar.style.display  = 'none';
      return;
    }

    if (emptyState)      emptyState.style.display      = 'none';
    if (filterBar)       filterBar.style.display        = 'flex';
    if (organizeToolbar) organizeToolbar.style.display  = 'flex';

    if (grid) {
      grid.style.display = 'flex';
      // Preserve rearrange-mode class when re-rendering
      if (isRearrangeMode) grid.classList.add('is-rearranging');
      else grid.classList.remove('is-rearranging');
    }

    grid.innerHTML = '';
    const query = filterInput ? filterInput.value.toLowerCase().trim() : '';

    // Filter by search query AND by active folder (if one is selected)
    const filtered = state.exercises.filter(ex => {
      const matchesSearch  = ex.name.toLowerCase().includes(query);
      const matchesFolder  = activeFilterDayId === null || ex.workoutDayId === activeFilterDayId;
      return matchesSearch && matchesFolder;
    });

    const filterCount = document.getElementById('filterCount');
    if (filterCount) {
      const activeDayName = activeFilterDayId
        ? (state.workoutDays.find(d => d.id === activeFilterDayId)?.name || 'folder')
        : null;
      if (activeDayName) {
        filterCount.textContent = query
          ? `${filtered.length} of ${state.exercises.filter(ex => ex.workoutDayId === activeFilterDayId).length} in ${activeDayName}`
          : `${filtered.length} exercise${filtered.length !== 1 ? 's' : ''} in ${activeDayName}`;
      } else {
        filterCount.textContent = query
    ...
    // When a specific folder is active, just render cards without group headers
    if (activeFilterDayId !== null) {
      if (filtered.length === 0) {
        const hint = document.createElement('p');
        hint.style.cssText = 'color:var(--text-muted);font-size:.88rem;padding:24px 0;text-align:center;';
        const folderName = state.workoutDays.find(d => d.id === activeFilterDayId)?.name || 'this folder';
        hint.textContent = `No exercises in "${folderName}" yet. Click Rearrange to drag exercises here.`;
        grid.appendChild(hint);
      } else {
        filtered.forEach(ex => grid.appendChild(createExerciseCardElement(ex)));
      }
      return;
    }

    // "All" view: group exercises by workout day (same as before)
  ...
  function createExerciseCardElement(ex) {
    ...
    // In rearrange mode show a day-picker; in normal mode show the chevron
    const daySelectOptions = state.workoutDays.map(d =>
      `<option value="${d.id}" ${ex.workoutDayId === d.id ? 'selected' : ''}>${escapeHtml(d.name)}</option>`
    ).join('');

    const headerRightHtml = isRearrangeMode
      ? `<div class="card-day-select-wrap">
           <select class="card-day-select" title="Assign to workout day" aria-label="Assign exercise to workout day">
             <option value="">— Ungrouped —</option>
             ${daySelectOptions}
           </select>
         </div>`
    ...
    const dragHandleHtml = isRearrangeMode
      ? `<div class="card-drag-handle" title="Drag to a folder" aria-hidden="true">
           <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5">
    ...
    card.innerHTML = `
      <!-- COLLAPSED HEADER: always visible, click to expand/collapse (disabled in rearrange mode) -->
      <div class="exercise-card-header-row"${isRearrangeMode ? '' : ' role="button" tabindex="0" aria-expanded="false"'}>
        ${dragHandleHtml}
  ...
  function bindCardEvents(card, ex) {
    ...
    // Drag-and-drop onto folder sidebar — only in rearrange mode
    if (isRearrangeMode) {
      ...
      card.addEventListener('dragstart', (e) => {
        ...
        // Allow drag handle to be grabbed without expanding card
        e.stopPropagation();
      });
    ...
    // Accordion toggle — only in normal (locked) mode
    if (headerRow && !isRearrangeMode) {
    ...
    // Day-select in rearrange mode — immediately saves and re-renders
    const cardDaySelect = card.querySelector('.card-day-select');
    if (cardDaySelect) {
      cardDaySelect.addEventListener('change', function (e) {
        // Prevent event bubbling from reaching any header-row handler
        e.stopPropagation();
        const newDayId = this.value || null;
        ex.workoutDayId = newDayId;
        saveState();
        const dayName = newDayId
          ? (state.workoutDays.find(d => d.id === newDayId)?.name || 'a day')
          : 'Ungrouped';
        showToast(`Moved "${ex.name}" → ${dayName}`, 'success');
        renderExercises();
      });

      // Prevent clicks on the select from propagating to the header row
      cardDaySelect.addEventListener('click', e => e.stopPropagation());
      cardDaySelect.addEventListener('mousedown', e => e.stopPropagation());
    }
  ...
  function initApp() {
    ...
    // Quick "Add Workout Day" button in the main toolbar
    const addDayQuickBtn = document.getElementById('addWorkoutDayQuickBtn');
    if (addDayQuickBtn) {
      addDayQuickBtn.addEventListener('click', () => {
        const name = window.prompt(
          'Enter a name for the new workout day:\n(e.g. Push, Pull, Legs, Upper Body, Monday…)'
        );
        if (name && name.trim()) {
          addWorkoutDay(name.trim());
        }
      });
    }

    // Rearrange / Finish buttons
    const rearrangeBtn      = document.getElementById('rearrangeBtn');
    const finishRearrangeBtn = document.getElementById('finishRearrangeBtn');
    if (rearrangeBtn)       rearrangeBtn.addEventListener('click', enterRearrangeMode);
    if (finishRearrangeBtn) finishRearrangeBtn.addEventListener('click', exitRearrangeMode);
